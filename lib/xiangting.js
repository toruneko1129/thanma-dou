...
/*
 *4枚使い七対子を扱えるモードを追加
 *usa_qidui: 4枚使い七対子を許容するかのフラグ default -> false
 */
function xiangting_qidui(shoupai, usa_qidui = false) {
    ...
    if (usa_qidui) {
        for (let s of ['m','p','s','z']) {
            let bingpai = shoupai._bingpai[s];
            for (let n = 1; n < bingpai.length; n++) {
                if      (bingpai[n] >= 4) n_duizi += 2;
                else if (bingpai[n] == 3) n_duizi++, n_guli++;
...
function xiangting(shoupai, usa_qidui = false) {
    return Math.min(
        xiangting_yiban(shoupai),
        xiangting_guoshi(shoupai),
        xiangting_qidui(shoupai, usa_qidui)
    );
}

function tingpai(shoupai, f_xiangting = xiangting, usa_qidui = false) {

    if (shoupai._zimo) return null;

    let pai = [];
    let n_xiangting = f_xiangting(shoupai, usa_qidui);
    for (let s of ['m','p','s','z']) {
        let bingpai = shoupai._bingpai[s];
        for (let n = 1; n < bingpai.length; n++) {
            if (bingpai[n] >= 4) continue;
            bingpai[n]++;
            if (f_xiangting(shoupai, usa_qidui) < n_xiangting) pai.push(s+n);
            bingpai[n]--;
        }
    }
    return pai;
